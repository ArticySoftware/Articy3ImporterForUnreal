...
void FArticyObjectDef::GenerateCode(CodeFileGenerator& header, const UArticyImportData* Data) const
{
	...
	{
		UE_LOG(LogArticyImporter, Log, TEXT("Skipped import of %s as it is a predefined type (%s)."), *GetOriginalType().ToString(), *Data->GetObjectDefs().GetCppType(GetOriginalType(), Data, false));
		return;
	}
...
void FArticyObjectDef::InitializeModel(UArticyPrimitive* Model, const FArticyModelDef& Vals, const UArticyImportData* Data) const
{
	if(DefType == EObjectDefType::Enum)
	{
		...
		UE_LOG(LogArticyImporter, Error, TEXT("Cannot initialize type %s, as it is an enum type!"), *Type.ToString());
		return;
	}
...
void FArticyObjectDefinitions::GatherScripts(const FArticyModelDef& Values, UArticyImportData* Data) const
{
	...
	{
		UE_LOG(LogArticyImporter, Error, TEXT("Model type %s for Model %s not found in definitions!"), *Values.GetType().ToString(), *Values.GetTechnicalName());
	}
}
